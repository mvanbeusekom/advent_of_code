...
Future<void> main() async {
  ...
  commonDivider = monkeys
      .map((Monkey monkey) => monkey.worryLevelDivider)
      .reduce((int a, int b) => (a * b) ~/ a.gcd(b));
...
class Monkey {
  ...
  void play() {
    items.forEach((int item) {
      inspects++;
      int updatedItem = _calculateWorryLevel(item) % commonDivider;
  ...
  int _calculateWorryLevel(int item) {
    ...
    if (worryLevelOperand != null) {
      operand = worryLevelOperand!;
    }
