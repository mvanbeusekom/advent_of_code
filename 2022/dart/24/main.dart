...
Future<void> main() async {
  ...
  final Node target = findNode(graph, targetCoordinate)!;

  late int shortest = kMaxInt;

  for (int j = 0; j < map.length; j++) {
    for (int i = 0; i < map.length; i++) {
      if (map[j][i] != 'a'.codeUnitAt(0)) {
        continue;
      }

      final Node start = findNode(graph, Coordinate(x: i, y: j))!;

      Dijkstra.execute(graph, start);

      final Node targetNode =
          graph.singleWhere((Node n) => n.value == target.value);

      shortest = targetNode.weight < shortest
          ? targetNode.weight
          : shortest;
    }
  }

  print('Least amount of steps: ${shortest}');
}
